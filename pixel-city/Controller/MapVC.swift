...
class MapVC: UIViewController, UIGestureRecognizerDelegate {
    ...
    let authoraziationStatus = CLLocationManager.authorizationStatus()
    ...
    @IBAction func centerMapButtonWasPressed(_ sender: Any) {
        if authoraziationStatus == .authorizedAlways || authoraziationStatus == .authorizedWhenInUse {
...
extension MapVC: MKMapViewDelegate {
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        //Dont change pin color for user location
    ...
    func centerMapOnUserLocation() {
        ...
        //Zoom where the user is
        let coordinateRegion  = MKCoordinateRegionMakeWithDistance(coordinate, regionRadius * 2.0, regionRadius * 2.0)
        mapView.setRegion(coordinateRegion, animated: true)
    }
    
    @objc func dropPin(sender: UITapGestureRecognizer) {
        //remove the other pins
        removePin()
        removeSpinner()
        removeProgressLbl()
        cancelAllSessions()
    ...
    func retrieveImages(handler: @escaping (_ status: Bool) -> ()) {
        for url in imageUrlArray {
            Alamofire.request(url).responseImage(completionHandler: { (response) in
                guard let image = response.result.value  else {  return handler(true)}
...
extension MapVC: CLLocationManagerDelegate {
    func configureLocationServices() {
        if authoraziationStatus == .notDetermined {
